...
alter table public.organization_ai_settings
  add constraint organization_ai_settings_provider_check
    check (provider in ('vercel', 'openrouter', 'theproxy'));
...
begin
  ...
  if p_provider not in ('vercel', 'openrouter', 'theproxy') then
  ...
  if p_provider = 'theproxy' and p_model_id <> 'gpt-5.5' then
    raise exception 'Modelo inválido para TheProxy.';
  end if;
