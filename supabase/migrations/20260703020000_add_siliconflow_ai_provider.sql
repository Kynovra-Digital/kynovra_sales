...
alter table public.organization_ai_settings
  add constraint organization_ai_settings_provider_check
    check (provider in ('vercel', 'openrouter', 'theproxy', 'agentrouter', 'siliconflow'));
...
begin
  ...
  if p_provider not in ('vercel', 'openrouter', 'theproxy', 'agentrouter', 'siliconflow') then
  ...
  if p_provider in ('theproxy', 'agentrouter') and p_model_id <> 'gpt-5.5' then
    raise exception 'Modelo inválido para o provedor selecionado.';
  end if;

  if p_provider = 'siliconflow' and p_model_id <> 'Qwen/Qwen3-32B' then
